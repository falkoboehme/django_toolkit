...
class UserBasedQuerysetCreatorMixin:
    """Handles creation and synchronization of project user_based_queryset.py"""

    _registry: Dict[str, Dict] = {}
    project_name: str = ""

    def _auto_create_user_based_queryset(self) -> set:
        files = set()
        app_class_names: list[str] = []

        for app_label in sorted(self._registry.keys()):
            app_file = self._auto_create_app_user_based_queryset(app_label)
            files.add(app_file) if app_file else None
            app_class_names.append(self._get_app_queryset_class_name(app_label))

        project_file = self._auto_create_project_user_based_queryset(app_class_names)
        files.add(project_file) if project_file else None

        return files

    def _auto_create_app_user_based_queryset(self, app_label: str):
        app_file_path = f"{app_label}/user_based_queryset.py"
        ...
        create_file(
            ...
            content=(
                ...
                "\n"
                "\n"
                f"class {app_class_name}(UserBasedQueryset):\n"
                "    pass\n"
            ),
        )
    ...
    def _auto_create_project_user_based_queryset(self, app_class_names: list[str]):
        project_file_path = f"{self.project_name}/user_based_queryset.py"
        imports = "\n".join(
            [
                "from django_toolkit.models import UserBasedQueryset",
                *[
                    f"from {app_label}.user_based_queryset import {self._get_app_queryset_class_name(app_label)}"
                    for app_label in sorted(self._registry.keys())
                ],
            ]
        )

        inheritance = ", ".join(app_class_names + ["UserBasedQueryset"])
        content = (
            f"{imports}\n"
            f"\n"
            f"\n"
            f"class ProjectUserBasedQueryset({inheritance}):\n"
            f"    pass\n"
        )
    ...
    def _get_app_queryset_class_name(app_label: str) -> str:
        ...
        return f"{normalized}UserBasedQueryset"

    @staticmethod
    def _ensure_app_class(content: str, class_name: str) -> str:
        class_signature = f"class {class_name}(UserBasedQueryset):"
        ...
        return (
            content.rstrip()
            + "\n\n\n"
            + f"class {class_name}(UserBasedQueryset):\n"
            + "    pass\n"
        )
    ...
    def _insert_method_into_app_class(content: str, class_name: str, method_name: str) -> str:
        class_anchor = f"class {class_name}(UserBasedQueryset):\n"
        method_block = (
            f"\n"
            f"    def {method_name}(self, queryset, user):\n"
