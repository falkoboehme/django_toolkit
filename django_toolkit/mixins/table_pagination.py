...
class EnhancedPaginator(Paginator):
    ...
    def __init__(self, object_list, per_page, orphans=None, **kwargs):
        ...
        try:
            ...
            if per_page < 1:
                per_page = settings.FAST_DEV_PAGINATE_COUNT
        except ValueError:
            per_page = settings.FAST_DEV_PAGINATE_COUNT
...
class TablePaginationMixin:
    def get_table_pagination(self, table):
        pagination = {
            ...
            'per_page': get_paginate_count(self.request)    # type: ignore
        }
        return pagination
...
def get_paginate_count(request):
    return min(settings.DT_ITEMS_PER_PAGE_DEFAULT, settings.DT_ITEMS_PER_PAGE_MAX)
